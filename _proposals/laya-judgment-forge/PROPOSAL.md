...
## 8. P2 v2 终局（2026-09-24 03:15 落档）——三门全 PASS，微调检查点在役

**v1 检查点验收（9-23 22:38）**：E1 PASS（82.7% 已反超 Jev）/E2 HOLD（fp 0.667）/E3 HOLD（sep 0.028）→ 自动回滚（保守纪律正确执行）。
**v2 夜训**（9-23 22:50 → 9-24 03:15，3 epochs 4.4h，loss 1.01→0.56→0.42，VRAM 峰值 6.49GB）→ 回归门三门全 PASS → `checkpoint-ft-v1_v2` 热换在役（active_checkpoint.txt）。

| 门（原措辞重放） | Jev 基线 | laya v2 微调 | 判 |
|---|---|---|---|
| E1 意图 acc | 71.7%（1.28s/题） | **88.3%（+16.5pp）**，53ms/题=24×，conf 对0.74/错0.28 | **PASS** |
| E2 待办 recall/fp@0.5 | 1.00 / 0.00 | **1.00 / 0.00**（三阈值全同 Jev） | **PASS** |
