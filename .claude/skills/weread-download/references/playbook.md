...
节流 3.5~8s/章 + 阅读间歇下（2026-09-16 收紧），大书 15~45 分钟/本是正常
水位（收紧前实测：42 章 14.9min、72 章 16.5min、202 章 37min；收紧后约 2×）。
后台任务输出长时间空白多为管道缓冲，不是卡死。判活三件套：
...
日限 2 本 / 书间 ≥300s / 节流+阅读间歇内置勿改 / 搜索间隔 ≥25s 且日限 8 次 /
登录探测优先 / 只读接口 / 熔断即停 / 风控零容忍。
放宽任何一条前必须问用户——收紧（更保守）可自主执行。

## 风控应对（2026-09-16 实战预案）

**背景**：连续 4 天 3 本/日 + 单晨 10 次搜索连发后，用户账号收到风控提醒。
已落地收紧：节流 (1.5,3.5)→(3.5,8.0)s、新增每 45~70 章长歇 60~150s、
