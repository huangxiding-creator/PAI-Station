...
from paistation.cx.search import aggregate_by_file, keyword_window, search  # noqa: E402

INDEX = REPO / "data/local_index/index.db"


def main() -> int:
    ap = argparse.ArgumentParser(description="全盘文件关键词检索（FTS5）")
    ap.add_argument("query", help="关键词，空格分隔=AND")
    ap.add_argument("--files", type=int, default=20, help="最多显示文件数")
    ap.add_argument("--samples", type=int, default=2, help="每文件样本行数")
    ap.add_argument("--limit", type=int, default=500, help="FTS 命中块上限")
    a = ap.parse_args()

    if not INDEX.exists():
        print(f"索引不存在：{INDEX}")
        return 1
    rows = search(INDEX, a.query, limit=a.limit)
    agg = aggregate_by_file(rows)
    total = sum(len(v) for _, v in agg)
    print(f"「{a.query}」→ {total} 命中块 / {len(agg)} 文件（块上限 {a.limit}）")
    words = a.query.split()
