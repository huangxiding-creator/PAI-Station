...
| **P4 外部源** | 手机备份 / 邮件 / 支付账单 | **需你配合**：手机连机备份、账单官方导出；SRUM/Prefetch 需管理员一次性授权 | 你有空时逐项挂起清单 |
...
**六维框架为工程顶层方法（2026-09-16 用户批准）**：**事实 × 行为 × 关系 × 观点 × 节律 × 演化**。落地为 `src/paistation/cx/dimensions.py`：每维登记数据源映射 + L0-L4 成熟度等级（L0无数据→L1数据在位→L2管线化→L3融合可检索→L4可评测）+ 量化探针；`tools/cx_dimensions.py` 随时产出六维记分卡快照（基线：2026-09-16，事实/行为 L2、关系/观点/节律/演化 L1，主时间轴 9298 事件）。所有 P0-P4 工作最终以"把某维推到 L3/L4"为验收口径。
...
### P4 外部源（挂起清单，你有空逐项触发）
- 手机：pymobiledevice3 备份 → pyiosbackup 解析（通话/短信/相册 EXIF/APP 清单）；安卓走官方备份+seednaut
- 健康/带走：Apple Health export.zip / Google Takeout → 解析器全套现成
- 邮件：你惯用网页邮箱则走 IMAP（imapsync 增量拉）或官方 Takeout；无 Outlook 桌面端
- 支付：微信支付/支付宝官方导出 → 订阅清点+消费结构
